...
%% Fix the stack descriptors of calls with arguments that are passed to the
%% stack by subtracting number of stack arguments from the frame size and from
%% the offset of the roots
...
fix_sdescs([{Offset, Arity} | Rest], SDescs) ->
  case lists:keyfind(Offset, 2, SDescs) of
    ...
    {?SDESC, Offset, SDesc} ->
      ...
      RootsList = tuple_to_list(Roots),
      FixedRoots = list_to_tuple(lists:map(DecRoot, RootsList)),
      FixedSDesc = {?SDESC, Offset, {ExnHandler, FrameSize - Arity, StkArity,
                                     FixedRoots}},
